...
// error here, at import time, not at `nautilus check` three commits later.
...
func ManifestYAML(m modbus.Manifest, command string) []byte {
	...
	b.WriteString("# Validated offline: `nautilus check` builds the block-read plan from this\n")
...
// nil — `nautilus modbus tags` without --map re-derives names and roles
...
func TagsYAML(m modbus.Manifest, meta map[string]TagMeta, skip []string) ([]byte, error) {
	header := []string{
		"Generated by `nautilus modbus import` from the device map. Do not edit —",
		"re-run the import, or `nautilus modbus tags modbus_manifest.yaml --map",
...
// for `nautilus modbus tags --map`: the manifest names the tags, the map
