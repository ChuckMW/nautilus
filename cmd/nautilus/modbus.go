...
const modbusUsage = `nautilus modbus — Modbus TCP tools

Usage:
  nautilus modbus import --map devices.yaml [flags]
  ...
  nautilus modbus tags <modbus_manifest.yaml> [flags]
...
func runModbus(args []string) int {
	...
	default:
		fmt.Fprintf(os.Stderr, "nautilus modbus: unknown subcommand %q\n\n%s", args[0], modbusUsage)
		return 2
	}
}
...
func runModbusImport(args []string) int {
	...
	if *mapPath == "" {
		fmt.Fprintln(os.Stderr, "nautilus modbus import: --map is required")
		return 2
	}

	raw, err := os.ReadFile(*mapPath)
	if err != nil {
		fmt.Fprintln(os.Stderr, "nautilus modbus import:", err)
		return 1
	}
	dm, err := codegen.ParseDeviceMap(raw)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nautilus modbus import: %s: %v\n", *mapPath, err)
		return 1
	}
	...
	command := "nautilus modbus import --map " + filepath.Base(*mapPath)
	if *instance != "" {
...
func runModbusTags(args []string) int {
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: nautilus modbus tags [-o tags/modbus.yaml] [--map devices.yaml] <modbus_manifest.yaml>")
		return 2
	}
	raw, err := os.ReadFile(fs.Arg(0))
	if err != nil {
		fmt.Fprintln(os.Stderr, "nautilus modbus tags:", err)
		return 1
	}
	m, err := modbus.ParseManifest(raw)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nautilus modbus tags: %s: %v\n", fs.Arg(0), err)
		return 1
	}
...
func runModbusBrowse(args []string) int {
	...
	if *host == "" {
		fmt.Fprintln(os.Stderr, "nautilus modbus browse: --host is required")
		return 2
	}
	switch *table {
	case modbus.TableHolding, modbus.TableInput, modbus.TableCoil, modbus.TableDiscrete:
	default:
		fmt.Fprintf(os.Stderr, "nautilus modbus browse: unknown table %q (want holding, input, coil or discrete)\n", *table)
		return 2
	}
	if *unit < 0 || *unit > 255 {
		fmt.Fprintln(os.Stderr, "nautilus modbus browse: --unit must be 0..255")
		return 2
	}
	if *from < 0 || *count < 1 || *from+*count > 0x10000 {
		fmt.Fprintln(os.Stderr, "nautilus modbus browse: --from/--count must fit the 16-bit address space")
		return 2
	}
	var f modbus.Format
	if *format != "" {
		...
		if f, err = modbus.ParseFormat(*format); err != nil {
			fmt.Fprintln(os.Stderr, "nautilus modbus browse:", err)
			return 2
		}
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nautilus modbus browse: cannot connect to %s: %v\n", addr, err)
		return 1
	}
	defer cl.Close()
...
func browseError(err error, unit uint8, timeout time.Duration) int {
	...
	case errors.As(err, &exc):
		fmt.Fprintf(os.Stderr, "nautilus modbus browse: %v\n"+
	...
	case errors.As(err, &nerr) && nerr.Timeout():
		fmt.Fprintf(os.Stderr, "nautilus modbus browse: no answer from unit %d within %s —\n"+
...
func runModbusServe(args []string) int {
	...
	if *manifest == "" {
		fmt.Fprintln(os.Stderr, "nautilus modbus serve: --manifest is required")
		return 2
	}
	raw, err := os.ReadFile(*manifest)
	if err != nil {
		fmt.Fprintln(os.Stderr, "nautilus modbus serve:", err)
		return 1
	}
	m, err := modbus.ParseManifest(raw)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nautilus modbus serve: %s: %v\n", *manifest, err)
		return 1
	}
