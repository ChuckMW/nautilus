...
driver, status, `slave/`, `codegen/`), `nautilus modbus import|browse|serve|tags`,
...
`nautilus modbus import|browse|serve`, mirroring `eip` and `sparkplug-host` end
...
modbus/
  ...
  codegen/      `nautilus modbus import` → modbus_manifest.yaml + tags/modbus.yaml
  slave/        in-repo Modbus TCP server (tests; `nautilus modbus serve` bench)
```
...
- `New` builds the read plan and validates addresses/formats offline —
  `nautilus check` and `build` pass with no device in sight (the
...
resulting count; `nautilus modbus import --plan` prints the plan so a
...
```yaml
# Generated by `nautilus modbus import --map devices.yaml`
sources:
  - id: FTIR_I
    host: 192.0.2.51
    port: 502
...
3. **Golden generator test** (`cmd/nautilus/modbus_test.go`):
