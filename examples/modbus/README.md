...
```sh
nautilus modbus import --map devices.yaml    # regenerate the committed files
nautilus test -m memory.yaml .               # logic-only acceptance tests
nautilus modbus serve --manifest modbus_manifest.yaml --values seed.json
...
- `tags/modbus.yaml` — the ordinary tag file (`role: input|output`, unit,
  ...
  alone with `nautilus modbus tags modbus_manifest.yaml --map devices.yaml`.
...
memory driver (`nautilus test -m memory.yaml .`): `given:` writes the
...
```sh
nautilus modbus browse --host 127.0.0.1 --port 5020 --unit 3 --from 2000 --count 4
nautilus modbus browse --host 127.0.0.1 --port 5020 --unit 4 --word-order little --format float32 --count 8
```
...
and `nautilus run .` — the driver builds the same plan `--plan` printed and
